#ifndef QINGZHU_TOOLS_TRIPLANAR
#define QINGZHU_TOOLS_TRIPLANAR

#include "Packages/com.unity.render-pipelines.core/ShaderLibrary/Packing.hlsl"

float4 triplanar(float3 vertPos, float3 normal, float scale,TEXTURE2D_PARAM(albedoAlphaMap, sampler_albedoAlphaMap)) {
	...
	float4 colX = SAMPLE_TEXTURE2D (albedoAlphaMap,sampler_albedoAlphaMap, uvX);
	float4 colY = SAMPLE_TEXTURE2D (albedoAlphaMap,sampler_albedoAlphaMap, uvY);
	float4 colZ = SAMPLE_TEXTURE2D (albedoAlphaMap,sampler_albedoAlphaMap, uvZ);
...
float3 triplanarNormal(float3 vertPos, float3 normal, float3 scale, float2 offset, TEXTURE2D_PARAM(normalMap, sampler_normalMap)) {
	...
	float3 tangentNormalX = UnpackNormal(SAMPLE_TEXTURE2D(normalMap,sampler_normalMap, uvX));
	float3 tangentNormalY = UnpackNormal(SAMPLE_TEXTURE2D(normalMap,sampler_normalMap, uvY));
	float3 tangentNormalZ = UnpackNormal(SAMPLE_TEXTURE2D(normalMap,sampler_normalMap, uvZ));
...
//
// float3 triplanarNormalTangentSpace(float3 vertPos, float3 normal, float3 scale, float4 tangent, TEXTURE2D_PARAM(normalMap, sampler_normalMap)) {
...
// }
//
// float3 triplanarNormalTangentSpace(float3 vertPos, float3 normal, float3 scale, float2 offset, float4 tangent, TEXTURE2D_PARAM(normalMap, sampler_normalMap)) {
